...
{{/eq}}
        ...
        try
        {
            ...
            if ({{ toCamelCase schema.moduleName }}InDB.dataLang.value.includes({{ toCamelCase schema.moduleName }}.langId.value)) throw new ConflictException(`Error to upsert {{ schema.aggregateName }}, the id ${{ bracketOpen }}{{ toCamelCase schema.moduleName }}['id']['value']} already exist in database`);
        ...
        catch (error)
        {
            if (error instanceof NotFoundException)
            {
                ...
                await this.repository.upsert({{ toCamelCase schema.moduleName }}, { upsertOptions: cQMetadata?.repositoryOptions });
            }
        }

        // save new i18n record
        await this.repositoryI18n.upsert(
