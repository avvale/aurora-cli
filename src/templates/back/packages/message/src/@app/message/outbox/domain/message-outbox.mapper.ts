...
export class MessageOutboxMapper implements IMapper {
    ...
    private makeAggregate(
        ...
        return MessageOutbox.register(
            ...
            new MessageOutboxRowId(outbox.rowId, { undefinable: true }),
            ...
            new MessageOutboxCreatedAt(
                outbox.createdAt,
                { undefinable: true },
                { addTimezone: cQMetadata?.timezone },
            ),
            new MessageOutboxUpdatedAt(
                outbox.updatedAt,
                { undefinable: true },
                { addTimezone: cQMetadata?.timezone },
            ),
            new MessageOutboxDeletedAt(
                outbox.deletedAt,
                { undefinable: true },
                { addTimezone: cQMetadata?.timezone },
            ),
            this.options.eagerLoading
                ? new MessageMessageMapper({
                      eagerLoading: true,
                  }).mapModelToAggregate(outbox.message, cQMetadata)
                : undefined,
        );
    }
    ...
    private makeResponse(outbox: MessageOutbox): MessageOutboxResponse {
        ...
        return new MessageOutboxResponse(
            ...
            this.options.eagerLoading
                ? new MessageMessageMapper({
                      eagerLoading: true,
                  }).mapAggregateToResponse(outbox.message)
                : undefined,
        );
    }
}
