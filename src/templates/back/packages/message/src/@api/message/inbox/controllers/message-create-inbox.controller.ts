...
import { MessageCreateInboxDto, MessageCreateInboxHandler, MessageInboxDto } from '@api/message/inbox';
...
{
    constructor(
        private readonly handler: MessageCreateInboxHandler,
    ) {}

    @Post()
    @ApiOperation({ summary: 'Create inbox' })
    @ApiCreatedResponse({ description: 'The record has been successfully created.', type: MessageInboxDto })
    @TenantPolicy()
    async main(
