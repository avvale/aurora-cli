...
export class MessageUpdateMessageByIdHandler {
  ...
  async main(
    ...
    const message = await this.queryBus.ask(
      new MessageFindMessageByIdQuery(payload.id, constraint, {
        timezone,
      }),
    );

    if (message.status === MessageMessageStatus.SENT)
      throw new BadRequestException(
        'Messages that have been sent cannot be modified.',
      );
    ...
    if ('scopeRecipients' in dataToUpdate)
      dataToUpdate.scopeRecipients = payload.scopeRecipients;
    if ('tagRecipients' in dataToUpdate)
      dataToUpdate.tagRecipients = payload.tagRecipients;
    if (
    ...
    const attachments = Array.isArray(payload.attachmentsInputFile)
      ? await Promise.all(
          payload.attachmentsInputFile.map(
            async (attachmentInputFile) =>
