...
export class MicrosoftGraphTransport implements Transport {
  ...
  async send(
    ...
    try {
      ...
      const userSegment = this.options.userId
        ? `/users/${this.options.userId}`
        : '/users'; // app-only does not have /me
  ...
  private toGraphMessage(data: Mail.Options): {
    message: any;
    saveToSentItems: boolean;
  } {
    // addresses
    const toRecipients = this.formatRecipients(
      data.to ? (Array.isArray(data.to) ? data.to : [data.to]) : [],
    );
    const ccRecipients = this.formatRecipients(
      data.cc ? (Array.isArray(data.cc) ? data.cc : [data.cc]) : [],
    );
    const bccRecipients = this.formatRecipients(
      data.bcc ? (Array.isArray(data.bcc) ? data.bcc : [data.bcc]) : [],
    );
