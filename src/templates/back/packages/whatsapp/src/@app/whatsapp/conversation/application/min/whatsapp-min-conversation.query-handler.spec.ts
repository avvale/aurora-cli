...
describe('WhatsappMinConversationQueryHandler', () => {
  ...
  beforeAll(async () => {
    ...
    queryHandler = module.get<WhatsappMinConversationQueryHandler>(
      WhatsappMinConversationQueryHandler,
    );
  ...
  describe('main', () => {
    ...
    test('should min inboxes', async () => {
      jest
        ...
        .mockImplementation(
          (column: string) =>
            new Promise((resolve) => resolve(repository.min(column))),
        );
      expect(
        await queryHandler.execute(new WhatsappMinConversationQuery('id')),
