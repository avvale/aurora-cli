...
export const mapMessageType = (type: string): WhatsappMessageType => {
  switch (type) {
    case 'audio':
    ...
    default:
      Logger.error(
        `Error to map message type, Error: Invalid message type ${type}. Include message type ${type} in message type enum database colum.`,
        'mapMessageType',
      );
      return WhatsappMessageType.UNKNOWN;
  }
