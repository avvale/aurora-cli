...
describe('WhatsappUpdateTimelinesController', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      imports: [],
      controllers: [WhatsappUpdateTimelinesController],
      providers: [
        {
    ...
    controller = module.get<WhatsappUpdateTimelinesController>(
      WhatsappUpdateTimelinesController,
    );
    handler = module.get<WhatsappUpdateTimelinesHandler>(
      WhatsappUpdateTimelinesHandler,
    );
  });
  ...
  describe('main', () => {
    ...
    test('should return a timelines updated', async () => {
      ...
      expect(await controller.main(whatsappMockTimelineData[0])).toBe(
