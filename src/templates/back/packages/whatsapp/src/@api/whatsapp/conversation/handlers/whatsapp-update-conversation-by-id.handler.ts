import {
  ...
  WhatsappUpdateConversationByIdInput,
} from '@api/graphql';
import {
  WhatsappConversationDto,
  WhatsappUpdateConversationByIdDto,
} from '@api/whatsapp/conversation';
import {
  WhatsappFindConversationByIdQuery,
  WhatsappUpdateConversationByIdCommand,
...
export class WhatsappUpdateConversationByIdHandler {
  ...
  async main(
    ...
    const conversation = await this.queryBus.ask(
      new WhatsappFindConversationByIdQuery(payload.id, constraint, {
        timezone,
      }),
    );
