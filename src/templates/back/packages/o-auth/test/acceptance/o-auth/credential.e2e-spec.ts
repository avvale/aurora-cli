...
describe('credential', () => {
  ...
  test('/REST:POST o-auth/credentials - Got 404, wrong application code', () => {
    return request(app.getHttpServer())
      ...
      .set(
        'Authorization',
        `Basic YXVyb3JhMjokMnkkMTAkRU9BL1NLRXdLUmdIUHc2NGtPNExaLjZveTViOGtsNkp6Vy9tQ1JPTWZTcTZTMzgvSWl5d0c=`,
      )
      .send({
        grantType: OAuthClientGrantType.PASSWORD,
        username: 'john.doe@gmail.com',
  ...
  test('/REST:POST o-auth/credentials - Got 404, wrong application secret', () => {
    return request(app.getHttpServer())
      ...
      .set(
        'Authorization',
        `Basic YXVyb3JhOiQyeSQxMCRFT0EvU0tFd0tSZ0hQdzY0a080TFouNm95NWI4a2w2SnpXL21DUk9NZlNxNlMzOC9JaXl3`,
      )
      .send({
        grantType: OAuthClientGrantType.PASSWORD,
        username: 'john.doe@gmail.com',
    ...
    return request(app.getHttpServer())
      ...
      .then((res) => {
        expect(res.body.message).toContain(
          'Authorization header has not a valid value, current decode value is: ',
        );
      });
  });
  ...
  test('/REST:POST o-auth/credentials - Got 400, ApplicationAuthorizationHeader not defined', () => {
    return request(app.getHttpServer())
      ...
      .then((res) => {
        expect(res.body.message).toContain(
          'Value for ApplicationAuthorizationHeader must be defined, can not be undefined',
        );
      });
  });
  ...
  test('/REST:POST o-auth/credentials - Got 400, username property not defined', () => {
    return request(app.getHttpServer())
      ...
      .then((res) => {
        expect(res.body.message).toContain(
          'Value for UserUsername must be defined, can not be undefined',
        );
      });
  });
  ...
  test('/REST:POST o-auth/credentials - Got 400, password property not defined', () => {
    return request(app.getHttpServer())
      ...
      .then((res) => {
        expect(res.body.message).toContain(
          'Value for UserPassword must be defined, can not be undefined',
        );
      });
  });
  ...
  test('/GraphQL oAuthCreateCredentials - Got 201, accessToken and refreshToken obtained', () => {
    return request(app.getHttpServer())
      ...
      .expect(200)
      .then((res) => {
        expect(res.body.data.oAuthCreateCredentials).toHaveProperty(
          'accessToken',
        );
        expect(res.body.data.oAuthCreateCredentials).toHaveProperty(
          'refreshToken',
  ...
  test('/GraphQL oAuthCreateCredentials - Got 401, Unauthorized to access by wrong password', () => {
    return request(app.getHttpServer())
      ...
      .then((res) => {
        ...
        expect(res.body.errors[0].extensions.response.message).toContain(
          'Unauthorized',
        );
      });
  });
    ...
    return request(app.getHttpServer())
      ...
      .then((res) => {
        expect(res.body.errors[0].message).toContain(
          'Variable "$payload" got invalid value',
        );
      });
  });
