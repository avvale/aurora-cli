...
describe('OAuthFindApplicationClientByIdController', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      imports: [],
      controllers: [OAuthFindApplicationClientByIdController],
      providers: [
        {
    ...
    controller = module.get<OAuthFindApplicationClientByIdController>(
      OAuthFindApplicationClientByIdController,
    );
    handler = module.get<OAuthFindApplicationClientByIdHandler>(
      OAuthFindApplicationClientByIdHandler,
    );
  });
  ...
  describe('main', () => {
    ...
    test('should return an applicationClient by id', async () => {
      ...
      expect(await controller.main(oAuthMockApplicationClientData[0].id)).toBe(
