...
import { OAuthCreateAccessTokenCommand, OAuthFindAccessTokenQuery } from '@app/o-auth/access-token';
...
import { AuditingMeta, ICommandBus, IQueryBus, Utils } from '@aurorajs.dev/core';
...
export class OAuthCreateImpersonalizeCredentialsHandler
{
    ...
    ): Promise<OAuthCredentials | OAuthCredentialsDto>
    {
        ...
        {
            // get account
            const account = await this.queryBus.ask(new IamFindAccountByIdQuery(accountId));

            // get client
            const client = await this.queryBus.ask(new OAuthFindClientByIdQuery(account.clientId));
