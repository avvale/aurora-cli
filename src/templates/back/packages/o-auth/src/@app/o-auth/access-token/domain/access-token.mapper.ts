...
export class AccessTokenMapper implements IMapper
{
    ...
    private makeAggregate(accessToken: LiteralObject, cQMetadata?: CQMetadata): OAuthAccessToken
    {
        return OAuthAccessToken.register(
            ...
            this.options.eagerLoading ? new RefreshTokenMapper({ eagerLoading: true }).mapModelToAggregate(accessToken.refreshToken, cQMetadata) : undefined,
            this.options.eagerLoading ? new ClientMapper({ eagerLoading: true }).mapModelToAggregate(accessToken.client, cQMetadata) : undefined,
        );
    }
