...
export class OAuthRefreshTokenMapper implements IMapper {
    ...
    mapAggregatesToResponses(
        ...
        return refreshTokens.map((refreshToken) =>
            this.makeResponse(refreshToken),
        );
    }
    ...
    private makeAggregate(
        ...
        return OAuthRefreshToken.register(
            ...
            new OAuthRefreshTokenCreatedAt(
                refreshToken.createdAt,
                { undefinable: true },
                { addTimezone: cQMetadata?.timezone },
            ),
            new OAuthRefreshTokenUpdatedAt(
                refreshToken.updatedAt,
                { undefinable: true },
                { addTimezone: cQMetadata?.timezone },
            ),
            new OAuthRefreshTokenDeletedAt(
            ...
            this.options.eagerLoading
                ? new OAuthAccessTokenMapper({
                      eagerLoading: true,
                  }).mapModelToAggregate(refreshToken.accessToken, cQMetadata)
                : undefined,
        );
    }
    ...
    private makeResponse(
        ...
        return new OAuthRefreshTokenResponse(
            ...
            this.options.eagerLoading
                ? new OAuthAccessTokenMapper({
                      eagerLoading: true,
                  }).mapAggregateToResponse(refreshToken.accessToken)
                : undefined,
        );
    }
}
