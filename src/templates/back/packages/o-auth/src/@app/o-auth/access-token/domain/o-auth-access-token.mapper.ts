...
export class OAuthAccessTokenMapper implements IMapper
{
     ...
     * @param accessToken
     */
    mapAggregateToResponse(accessToken: OAuthAccessToken): OAuthAccessTokenResponse
    {
     ...
     * @param accessTokens
     */
    mapAggregatesToResponses(accessTokens: OAuthAccessToken[]): OAuthAccessTokenResponse[]
    {
        if (!Array.isArray(accessTokens)) return;
    ...
    private makeAggregate(accessToken: LiteralObject, cQMetadata?: CQMetadata): OAuthAccessToken
    {
        return OAuthAccessToken.register(
            ...
            new OAuthAccessTokenClientId(accessToken.clientId, { undefinable: true }),
            new OAuthAccessTokenAccountId(accessToken.accountId, { undefinable: true }),
            new OAuthAccessTokenToken(accessToken.token, { undefinable: true }),
            new OAuthAccessTokenName(accessToken.name, { undefinable: true }),
            new OAuthAccessTokenIsRevoked(accessToken.isRevoked, { undefinable: true }),
            new OAuthAccessTokenExpiresAt(accessToken.expiresAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            new OAuthAccessTokenCreatedAt(accessToken.createdAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            new OAuthAccessTokenUpdatedAt(accessToken.updatedAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            new OAuthAccessTokenDeletedAt(accessToken.deletedAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            this.options.eagerLoading ? new OAuthRefreshTokenMapper({ eagerLoading: true }).mapModelToAggregate(accessToken.refreshToken, cQMetadata) : undefined,
            this.options.eagerLoading ? new OAuthClientMapper({ eagerLoading: true }).mapModelToAggregate(accessToken.client, cQMetadata) : undefined,
        );
    }

    private makeResponse(accessToken: OAuthAccessToken): OAuthAccessTokenResponse
    {
        if (!accessToken) return;
        ...
        return new OAuthAccessTokenResponse(
            ...
            this.options.eagerLoading ? new OAuthRefreshTokenMapper({ eagerLoading: true }).mapAggregateToResponse(accessToken.refreshToken) : undefined,
            this.options.eagerLoading ? new OAuthClientMapper({ eagerLoading: true }).mapAggregateToResponse(accessToken.client) : undefined,
        );
    }
}
