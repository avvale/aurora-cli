...
export class OAuthClientMapper implements IMapper
{
    ...
    private makeAggregate(client: LiteralObject, cQMetadata?: CQMetadata): OAuthClient
    {
        return OAuthClient.register(
            ...
            new OAuthClientScopeOptions(client.scopeOptions, { undefinable: true }),
            new OAuthClientExpiredAccessToken(client.expiredAccessToken, { undefinable: true }),
            new OAuthClientExpiredRefreshToken(client.expiredRefreshToken, { undefinable: true }),
            new OAuthClientIsActive(client.isActive, { undefinable: true }),
            new OAuthClientIsMaster(client.isMaster, { undefinable: true }),
            new OAuthClientApplicationIds(client.applicationIds, { undefinable: true }),
            new OAuthClientCreatedAt(client.createdAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            new OAuthClientUpdatedAt(client.updatedAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            new OAuthClientDeletedAt(client.deletedAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            this.options.eagerLoading ? new OAuthAccessTokenMapper({ eagerLoading: true }).mapModelsToAggregates(client.accessTokens, cQMetadata) : undefined,
            this.options.eagerLoading ? new OAuthApplicationMapper({ eagerLoading: true }).mapModelsToAggregates(client.applications, cQMetadata) : undefined,
        );
    }
    ...
    private makeResponse(client: OAuthClient): OAuthClientResponse
    {
        ...
        return new OAuthClientResponse(
            ...
            this.options.eagerLoading ? new OAuthAccessTokenMapper({ eagerLoading: true }).mapAggregatesToResponses(client.accessTokens) : undefined,
            this.options.eagerLoading ? new OAuthApplicationMapper({ eagerLoading: true }).mapAggregatesToResponses(client.applications) : undefined,
        );
    }
}
