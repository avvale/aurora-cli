...
describe('queue', () => {
    ...
    beforeAll(async () => {
        const module: TestingModule = await Test.createTestingModule({
            ...
            providers: [QueueManagerMockQueueSeeder],
        })
        ...
        queueRepository = module.get<QueueManagerIQueueRepository>(
            QueueManagerIQueueRepository,
        );
        queueSeeder = module.get<QueueManagerMockQueueSeeder>(
            QueueManagerMockQueueSeeder,
        );
        ...
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body.message).toContain(
                    'Value for QueueManagerQueueId must be defined, can not be null',
                );
            });
    });
    ...
    test('/REST:POST queue-manager/queue/create - Got 400 Conflict, QueuePrefix property can not to be null', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body.message).toContain(
                    'Value for QueueManagerQueuePrefix must be defined, can not be null',
                );
            });
    });
    ...
    test('/REST:POST queue-manager/queue/create - Got 400 Conflict, QueueName property can not to be null', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body.message).toContain(
                    'Value for QueueManagerQueueName must be defined, can not be null',
                );
            });
    });
    ...
    test('/REST:POST queue-manager/queue/create - Got 400 Conflict, QueueId property can not to be undefined', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body.message).toContain(
                    'Value for QueueManagerQueueId must be defined, can not be undefined',
                );
            });
    });
    ...
    test('/REST:POST queue-manager/queue/create - Got 400 Conflict, QueuePrefix property can not to be undefined', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body.message).toContain(
                    'Value for QueueManagerQueuePrefix must be defined, can not be undefined',
                );
            });
    });
    ...
    test('/REST:POST queue-manager/queue/create - Got 400 Conflict, QueueName property can not to be undefined', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body.message).toContain(
                    'Value for QueueManagerQueueName must be defined, can not be undefined',
                );
            });
    });
    ...
    test('/REST:POST queue-manager/queue/create - Got 400 Conflict, QueueId is not allowed, must be a length of 36', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body.message).toContain(
                    'Value for QueueManagerQueueId is not allowed, must be a length of 36',
                );
            });
    });
    ...
    test('/REST:POST queue-manager/queue/create - Got 400 Conflict, QueuePrefix is too large, has a maximum length of 63', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body.message).toContain(
                    'Value for QueueManagerQueuePrefix is too large, has a maximum length of 63',
                );
            });
    });
    ...
    test('/REST:POST queue-manager/queue/create - Got 400 Conflict, QueueName is too large, has a maximum length of 63', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body.message).toContain(
                    'Value for QueueManagerQueueName is too large, has a maximum length of 63',
                );
            });
    });
        ...
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body).toEqual({
                    ...
                    rows: queueSeeder.collectionResponse
                        .map((item) =>
    ...
    test('/REST:POST queue-manager/queues/get', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(res.body).toEqual(
                    queueSeeder.collectionResponse.map((item) =>
                        expect.objectContaining(
                            _.omit(item, [
                                'createdAt',
                                'updatedAt',
    ...
    test('/REST:POST queue-manager/queue/find/{id} - Got 404 Not Found', () => {
        return request(app.getHttpServer())
            .post(
                '/queue-manager/queue/find/3f7793a5-41bc-5154-ac67-221445c98d99',
            )
            .set('Accept', 'application/json')
            .expect(404);
    });
    ...
    test('/REST:POST queue-manager/queue/find/{id}', () => {
        return request(app.getHttpServer())
            .post(
                '/queue-manager/queue/find/5b19d6ac-4081-573b-96b3-56964d5326a8',
            )
            .set('Accept', 'application/json')
            .expect(200)
            .then((res) => {
    ...
    test('/REST:DELETE queue-manager/queue/delete/{id} - Got 404 Not Found', () => {
        return request(app.getHttpServer())
            .delete(
                '/queue-manager/queue/delete/1e92e502-8af7-5872-ab06-2baedc8b6250',
            )
            .set('Accept', 'application/json')
            .expect(404);
    });
    ...
    test('/REST:DELETE queue-manager/queue/delete/{id}', () => {
        return request(app.getHttpServer())
            .delete(
                '/queue-manager/queue/delete/5b19d6ac-4081-573b-96b3-56964d5326a8',
            )
            .set('Accept', 'application/json')
            .expect(200);
    });
        ...
        return request(app.getHttpServer())
            ...
            .then((res) => {
                for (const [
                    index,
                    value,
                ] of res.body.data.queueManagerGetQueues.entries()) {
                    expect(queueSeeder.collectionResponse[index]).toEqual(
                        expect.objectContaining(
                            _.omit(value, [
                                'createdAt',
                                'updatedAt',
    ...
    test('/GraphQL queueManagerCreateQueue', () => {
        return request(app.getHttpServer())
            ...
            .expect(200)
            .then((res) => {
                expect(res.body.data.queueManagerCreateQueue).toHaveProperty(
                    'id',
                    '5b19d6ac-4081-573b-96b3-56964d5326a8',
                );
            });
    ...
    test('/GraphQL queueManagerFindQueue', () => {
        return request(app.getHttpServer())
            ...
            .expect(200)
            .then((res) => {
                expect(res.body.data.queueManagerFindQueue.id).toStrictEqual(
                    '5b19d6ac-4081-573b-96b3-56964d5326a8',
                );
            });
    });
    ...
    test('/GraphQL queueManagerFindQueueById', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(
                    res.body.data.queueManagerFindQueueById.id,
    ...
    test('/GraphQL queueManagerUpdateQueueById', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(
                    res.body.data.queueManagerUpdateQueueById.id,
                ).toStrictEqual('5b19d6ac-4081-573b-96b3-56964d5326a8');
            });
    });
    ...
    test('/GraphQL queueManagerUpdateQueues', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(
                    res.body.data.queueManagerUpdateQueues[0].id,
                ).toStrictEqual('5b19d6ac-4081-573b-96b3-56964d5326a8');
            });
    });
    ...
    test('/GraphQL queueManagerDeleteQueueById', () => {
        return request(app.getHttpServer())
            ...
            .then((res) => {
                expect(
                    res.body.data.queueManagerDeleteQueueById.id,
                ).toStrictEqual('5b19d6ac-4081-573b-96b3-56964d5326a8');
            });
    });
