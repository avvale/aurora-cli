...
export class CommonCreateAdministrativeAreasLevel3CommandHandler
  implements ICommandHandler<CommonCreateAdministrativeAreasLevel3Command>
{
  constructor(
  ...
  async execute(
    command: CommonCreateAdministrativeAreasLevel3Command,
  ): Promise<void> {
    // call to use case and implements ValueObjects
    await this.createAdministrativeAreasLevel3Service.main(
      command.payload.map((administrativeAreaLevel3) => {
        return {
