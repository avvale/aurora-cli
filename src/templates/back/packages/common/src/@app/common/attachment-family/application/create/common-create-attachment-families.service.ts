import {
  CommonAddAttachmentFamiliesContextEvent,
...
export class CommonCreateAttachmentFamiliesService {
  ...
  async main(
    ...
    const aggregateAttachmentFamilies = payload.map((attachmentFamily) =>
    ...
    const attachmentFamiliesRegistered = this.publisher.mergeObjectContext(
      new CommonAddAttachmentFamiliesContextEvent(aggregateAttachmentFamilies),
    );
