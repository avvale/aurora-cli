...
export class CommonCreateAttachmentLibrariesCommandHandler
  implements ICommandHandler<CommonCreateAttachmentLibrariesCommand>
{
  constructor(
  ...
  async execute(
    command: CommonCreateAttachmentLibrariesCommand,
  ): Promise<void> {
    // call to use case and implements ValueObjects
    await this.createAttachmentLibrariesService.main(
      command.payload.map((attachmentLibrary) => {
        return {
