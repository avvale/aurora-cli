import {
  ...
  CommonRawSQLAdministrativeAreasLevel3Query,
...
export class CommonRawSQLAdministrativeAreasLevel3QueryHandler
  implements IQueryHandler<CommonRawSQLAdministrativeAreasLevel3Query>
{
  ...
  async execute(
    query: CommonRawSQLAdministrativeAreasLevel3Query,
  ): Promise<CommonAdministrativeAreaLevel3Response[]> {
    return this.mapper.mapAggregatesToResponses(
      await this.rawSQLAdministrativeAreasLevel3Service.main(
        query.rawSQL,
        query.cQMetadata,
      ),
    );
