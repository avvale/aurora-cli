...
export class CommonCreateAttachmentFamiliesCommandHandler
  implements ICommandHandler<CommonCreateAttachmentFamiliesCommand>
{
  constructor(
  ...
  async execute(command: CommonCreateAttachmentFamiliesCommand): Promise<void> {
    ...
    await this.createAttachmentFamiliesService.main(
      command.payload.map((attachmentFamily) => {
        return {
