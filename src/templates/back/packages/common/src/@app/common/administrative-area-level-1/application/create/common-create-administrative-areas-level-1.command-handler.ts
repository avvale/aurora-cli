...
export class CommonCreateAdministrativeAreasLevel1CommandHandler
  implements ICommandHandler<CommonCreateAdministrativeAreasLevel1Command>
{
  constructor(
  ...
  async execute(
    command: CommonCreateAdministrativeAreasLevel1Command,
  ): Promise<void> {
    // call to use case and implements ValueObjects
    await this.createAdministrativeAreasLevel1Service.main(
      command.payload.map((administrativeAreaLevel1) => {
        return {
