...
export class CommonRawSQLLangsQueryHandler
  implements IQueryHandler<CommonRawSQLLangsQuery>
{
  private readonly mapper: CommonLangMapper = new CommonLangMapper();

  constructor(private readonly rawSQLLangsService: CommonRawSQLLangsService) {}

  async execute(query: CommonRawSQLLangsQuery): Promise<CommonLangResponse[]> {
    return this.mapper.mapAggregatesToResponses(
      await this.rawSQLLangsService.main(query.rawSQL, query.cQMetadata),
    );
  }
}
