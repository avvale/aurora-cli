...
export class CommonUpsertCountryService {
  ...
  async main(
    ...
    try {
      ...
      if (countryInDB.availableLangs.value.includes(contentLanguage.id))
        throw new ConflictException(
          `Error to upsert CommonCountry, the id ${contentLanguage.id} already exist in database`,
        );

      // add new lang id to data lang field to upsert field
      country.availableLangs = new CommonCountryAvailableLangs(
        _.union(countryInDB.availableLangs.value, [contentLanguage.id]),
      );
      await this.repository.update(country, {
        dataFactory: (aggregate) =>
          _.pick(aggregate.toI18nDTO(), 'id', 'availableLangs'),
      ...
      if (error instanceof NotFoundException) {
        ...
        await this.repository.upsert(country, {
          upsertOptions: cQMetadata?.repositoryOptions,
        });
      }
    }

    const modelInDB = await this.repositoryI18n.find({
      queryStatement: {
        where: {
    ...
    await this.repositoryI18n.upsert(countryI18n, {
