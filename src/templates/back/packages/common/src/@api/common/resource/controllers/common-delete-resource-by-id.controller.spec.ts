...
describe('CommonDeleteResourceByIdController', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      imports: [],
      controllers: [CommonDeleteResourceByIdController],
      providers: [
        {
    ...
    controller = module.get<CommonDeleteResourceByIdController>(
      CommonDeleteResourceByIdController,
    );
  ...
  describe('main', () => {
    ...
    test('should return an resource deleted', async () => {
      ...
      expect(await controller.main(commonMockResourceData[0].id)).toBe(
