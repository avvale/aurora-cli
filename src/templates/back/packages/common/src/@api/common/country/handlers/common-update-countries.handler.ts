  ...
  async main(
    ...
    if (!contentLanguage)
      throw new BadRequestException(
        'To update a multi-language objects, the content-language header must be defined.',
      );

    await this.commandBus.dispatch(
      new CommonUpdateCountriesCommand(payload, queryStatement, constraint, {
        timezone,
        repositoryOptions: {
          auditing,
        },
