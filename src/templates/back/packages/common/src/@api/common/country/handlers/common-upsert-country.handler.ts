  ...
  async main(
    ...
    if (!contentLanguage)
      throw new BadRequestException(
        'To upsert a multi-language object, the content-language header must be defined.',
      );

    await this.commandBus.dispatch(
      new CommonUpsertCountryCommand(payload, {
        timezone,
        repositoryOptions: {
          auditing,
        },
    ...
    return await this.queryBus.ask(
      new CommonFindCountryByIdQuery(payload.id, constraint, {
        timezone,
      }),
    );
  }
