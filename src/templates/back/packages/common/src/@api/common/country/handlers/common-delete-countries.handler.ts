  ...
  async main(
    ...
    if (!contentLanguage)
      throw new BadRequestException(
        'To delete an multi-language objects, the content-language header must be defined.',
      );
    ...
    const countries = await this.queryBus.ask(
      new CommonGetCountriesQuery(queryStatement, constraint, {
        timezone,
      }),
    );

    await this.commandBus.dispatch(
      new CommonDeleteCountriesCommand(queryStatement, constraint, {
        timezone,
        repositoryOptions: {
          auditing,
        },
