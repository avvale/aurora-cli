...
export class CommonAdministrativeAreasCountryHandler {
    ...
    async main(
        ...
        let commonGetAdministrativeAreasLevel1 = [],
            commonGetAdministrativeAreasLevel2 = [],
            commonGetAdministrativeAreasLevel3 = [];
        ...
        const commonCountry = await this.queryBus.ask(
            new CommonFindCountryByIdQuery(countryId, constraint),
        );
        ...
        const hasAdministrativeAreas =
            Array.isArray(commonCountry.administrativeAreas) &&
            commonCountry.administrativeAreas.length !== 0;

        if (
            !hasAdministrativeAreas ||
            commonCountry.administrativeAreas?.includes(
                CommonAdministrativeArea.ADMINISTRATIVE_AREA_LEVEL_1,
            )
        ) {
            commonGetAdministrativeAreasLevel1 = await this.queryBus.ask(
        ...
        if (
            !hasAdministrativeAreas ||
            commonCountry.administrativeAreas?.includes(
                CommonAdministrativeArea.ADMINISTRATIVE_AREA_LEVEL_2,
            )
        ) {
            ...
            if (!administrativeAreaLevel1Id)
                delete whereStatement.administrativeAreaLevel1Id;

            commonGetAdministrativeAreasLevel2 = await this.queryBus.ask(
        ...
        if (
            !hasAdministrativeAreas ||
            commonCountry.administrativeAreas?.includes(
                CommonAdministrativeArea.ADMINISTRATIVE_AREA_LEVEL_3,
            )
        ) {
            const whereStatement = {
                countryId,
            ...
            if (!administrativeAreaLevel1Id)
                delete whereStatement.administrativeAreaLevel1Id;
            if (!administrativeAreaLevel2Id)
                delete whereStatement.administrativeAreaLevel2Id;

            commonGetAdministrativeAreasLevel3 = await this.queryBus.ask(
