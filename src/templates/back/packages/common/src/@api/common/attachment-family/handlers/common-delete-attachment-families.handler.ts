...
import {
  CommonDeleteAttachmentFamiliesCommand,
  CommonGetAttachmentFamiliesQuery,
...
export class CommonDeleteAttachmentFamiliesHandler {
  ...
  async main(
    ...
    const attachmentFamilies = await this.queryBus.ask(
      new CommonGetAttachmentFamiliesQuery(queryStatement, constraint, {
        timezone,
      }),
    );

    await this.commandBus.dispatch(
      new CommonDeleteAttachmentFamiliesCommand(queryStatement, constraint, {
        timezone,
        repositoryOptions: {
          auditing,
        },
