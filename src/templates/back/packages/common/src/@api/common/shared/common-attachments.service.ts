...
export class CommonAttachmentsService {
  ...
  async createUploadedAttachments(
    ...
    if (uploadedAttachmentLibraries.length > 0) {
      await this.commandBus.dispatch(
        new CommonCreateAttachmentLibrariesCommand(uploadedAttachmentLibraries),
      );
    }
  }
  ...
  > {
    ...
    if (!attachmentFamilies.some((attachmentFamily) => attachmentFamily.format))
      return attachments;
    ...
    for (const attachment of attachments) {
      ...
      const attachmentFamily = attachmentFamilies.find(
        (attachmentFamily) => attachmentFamily.id === attachment.familyId,
      );

      if (!attachmentFamily.format) continue;
      if (
        Utils.mimeFromExtension(attachmentFamily.format.toLowerCase()) ===
        attachment.mimetype
      )
        continue;
  ...
  async duplicateAttachmentsForNewI18nObject(
    ...
    const lang = langs.find(
      (lang) =>
        lang[this.coreGetSearchKeyLangService.get()] === contentLanguage,
    );
    ...
    for (const attachment of attachments) {
      ...
      const sourceAbsoluteDirectoryAttachmentPath = storagePublicAbsolutePath(
        attachment.relativePathSegments,
        attachment.filename,
      );
