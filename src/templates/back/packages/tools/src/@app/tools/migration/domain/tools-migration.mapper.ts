...
export class ToolsMigrationMapper implements IMapper {
  ...
  mapModelsToAggregates(
    migrations: LiteralObject[],
    cQMetadata?: CQMetadata,
  ): ToolsMigration[] {
    if (!Array.isArray(migrations)) return;

    return migrations.map((migration) =>
      this.makeAggregate(migration, cQMetadata),
    );
  }
  ...
  private makeAggregate(
    ...
    return ToolsMigration.register(
      ...
      new ToolsMigrationExecutedAt(
        migration.executedAt,
        { undefinable: true },
        { addTimezone: cQMetadata?.timezone },
      ),
      new ToolsMigrationCreatedAt(
        migration.createdAt,
        { undefinable: true },
        { addTimezone: cQMetadata?.timezone },
      ),
      new ToolsMigrationUpdatedAt(
        migration.updatedAt,
        { undefinable: true },
        { addTimezone: cQMetadata?.timezone },
      ),
      new ToolsMigrationDeletedAt(
        migration.deletedAt,
        { undefinable: true },
        { addTimezone: cQMetadata?.timezone },
      ),
