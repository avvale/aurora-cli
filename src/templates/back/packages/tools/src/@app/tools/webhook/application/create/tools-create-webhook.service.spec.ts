    ...
    test('should create a webhook and emit event', async () => {
      expect(
        await service.main({
          ...
          service: new ToolsWebhookService(toolsMockWebhookData[0].service),
          endpoint: new ToolsWebhookEndpoint(toolsMockWebhookData[0].endpoint),
          externalId: new ToolsWebhookExternalId(
            toolsMockWebhookData[0].externalId,
          ),
          events: new ToolsWebhookEvents(toolsMockWebhookData[0].events),
          secret: new ToolsWebhookSecret(toolsMockWebhookData[0].secret),
