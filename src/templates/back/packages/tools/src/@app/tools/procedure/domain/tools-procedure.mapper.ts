...
export class ToolsProcedureMapper implements IMapper
{
     ...
     * @param procedure
     */
    mapModelToAggregate(procedure: LiteralObject, cQMetadata?: CQMetadata): ToolsProcedure
    {
        if (!procedure) return;
     ...
     * @param procedures
     */
    mapModelsToAggregates(procedures: LiteralObject[], cQMetadata?: CQMetadata): ToolsProcedure[]
    {
        if (!Array.isArray(procedures)) return;

        return procedures.map(procedure => this.makeAggregate(procedure, cQMetadata));
    }
     ...
     * @param procedures
     */
    mapAggregatesToResponses(procedures: ToolsProcedure[]): ToolsProcedureResponse[]
    {
        if (!Array.isArray(procedures)) return;
    ...
    private makeAggregate(procedure: LiteralObject, cQMetadata?: CQMetadata): ToolsProcedure
    {
        return ToolsProcedure.register(
            ...
            new ToolsProcedureIsActive(procedure.isActive, { undefinable: true }),
            new ToolsProcedureIsInstalled(procedure.isInstalled, { undefinable: true }),
            new ToolsProcedureIsUpdated(procedure.isUpdated, { undefinable: true }),
            new ToolsProcedureUpScript(procedure.upScript, { undefinable: true }),
            new ToolsProcedureDownScript(procedure.downScript, { undefinable: true }),
            new ToolsProcedureSort(procedure.sort, { undefinable: true }),
            new ToolsProcedureExecutedAt(procedure.executedAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            new ToolsProcedureCheckedAt(procedure.checkedAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            new ToolsProcedureCreatedAt(procedure.createdAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            new ToolsProcedureUpdatedAt(procedure.updatedAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            new ToolsProcedureDeletedAt(procedure.deletedAt, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
        );
    }
