...
export class ToolsUpdateMigrationsCommandHandler implements ICommandHandler<ToolsUpdateMigrationsCommand>
{
    constructor(
    ...
    async execute(command: ToolsUpdateMigrationsCommand): Promise<void>
    {
        ...
        await this.updateMigrationsService.main(
            {
                id: new ToolsMigrationId(command.payload.id, { undefinable: true }),
                name: new ToolsMigrationName(command.payload.name, { undefinable: true }),
                version: new ToolsMigrationVersion(command.payload.version, { undefinable: true }),
                isActive: new ToolsMigrationIsActive(command.payload.isActive, { undefinable: true }),
                isExecuted: new ToolsMigrationIsExecuted(command.payload.isExecuted, { undefinable: true }),
