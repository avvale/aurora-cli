...
export class ToolsLoadProceduresService {
  ...
  async onApplicationBootstrap(): Promise<void> {
    // get all procedures
    const proceduresInDatabase = await this.queryBus.ask(
      new ToolsGetProceduresQuery(),
    );

    const proceduresToStorage = [];
    for (const procedure of procedures) {
      const procedureInDatabase = proceduresInDatabase.find(
        (procedureInDatabase) => procedureInDatabase.id === procedure.id,
      );
      ...
      if (procedureInDatabase) {
        proceduresToStorage.push({
          ...
          isUpdated:
            procedureInDatabase.isExecuted &&
            procedureInDatabase.hash &&
            `sha1:${Encrypt.sha1(procedure.upScript)}` !==
              procedureInDatabase.hash
              ? true
              : procedureInDatabase.isUpdated,
        });
      } else {
        proceduresToStorage.push({
          ...procedure,
    ...
    await this.commandBus.dispatch(
      new ToolsCreateProceduresCommand(proceduresToStorage, {
        repositoryOptions: {
          updateOnDuplicate: [
            'name',
            'type',
