...
// add the accountId of the current user to the constraint
// of a query to limit the results it has access to
export const AccountConstraint = ({
    ...
    ) =>
    {
        return {
            value( ...args: any[])
            {
                ...
                if (!account) throw new BadRequestException('To use @AccountConstraint() decorator need has @CurrentAccount() defined in properties of method');
                ...
                if (isArray)
                {
                    args[constraintIndex] = _.merge(args[constraintIndex], {
                        where: {
                            [targetProperty]: {
                                // must contain all the ids passed as argument
