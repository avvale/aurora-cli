...
export const createAccount = async ({
    ...
    if (!payload)
        throw new BadRequestException('payload parameter is required');
    ...
    const accounts = await queryBus.ask(
    ...
    const client = await queryBus.ask(
        new OAuthFindClientByIdQuery(payload.clientId, {
            include: [
                {
    ...
    // get roles
    const roles = await queryBus.ask(
    ...
    if (
        ...
        throw new ConflictException({
            message:
                'Your account does not have the required permissions to create an account with the specified roles.',
            statusCode: 401,
    ...
    if (payload.hasAddChildTenants) {
        ...
        for (const tenantId of payload.tenantIds) {
            ...
            for (const tenant of getNestedObjectsFromParentId(
                tenants,
                tenantId,
            )) {
        ...
        new IamCreateAccountCommand(
            {
                ...
                dApplicationCodes: client?.applications.map(
