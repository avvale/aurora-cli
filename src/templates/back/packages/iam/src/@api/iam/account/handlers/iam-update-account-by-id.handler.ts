...
export class IamUpdateAccountByIdHandler {
    ...
    async main(
        ...
        const accountToUpdate = await this.queryBus.ask(
            new IamFindAccountByIdQuery(payload.id, constraint, {
                timezone,
            }),
        );
        ...
        if ('roleIds' in dataToUpdate) {
            ...
            if (
                ...
                throw new ConflictException({
                    message:
                        'The account does not have the required permissions to update the account with the specified roles.',
                    statusCode: 401,
        ...
        if (payload.hasAddChildTenants) {
            ...
            if ('tenantIds' in dataToUpdate)
                dataToUpdate['tenantIds'] = payload.tenantIds;
        }
        ...
        if (accountToUpdate.type === IamAccountType.USER) {
            const user = await this.queryBus.ask(
                new IamFindUserByIdQuery(payload.user.id, constraint, {
                    timezone,
                }),
            );
