...
describe('IamCreateBoundedContextsController', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      controllers: [IamCreateBoundedContextsController],
      providers: [
        {
    ...
    controller = module.get<IamCreateBoundedContextsController>(
      IamCreateBoundedContextsController,
    );
  });
  ...
  describe('main', () => {
    ...
    test('should return an iamMockBoundedContextData created', async () => {
      expect(await controller.main(iamMockBoundedContextData)).toBe(undefined);
    });
  });
});
