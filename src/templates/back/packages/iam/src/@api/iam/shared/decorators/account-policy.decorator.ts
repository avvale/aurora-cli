      ...
      value(...args: any[]) {
        ...
        if (!account)
          throw new BadRequestException(
            'To use @AccountPolicy() decorator need has @CurrentAccount() defined in properties of method',
          );

        // always manage array of payloads
        const payload = Array.isArray(args[payloadIndex])
          ? args[payloadIndex]
          : [args[payloadIndex]];
        ...
        for (const item of payload) {
          if (!item[targetProperty])
            throw new BadRequestException(
              `The ${targetProperty} property not found in payload, maybe has to set payloadIndex or targetProperty arguments of AccountPolicy decorator`,
            );
          if (isArray) {
            if (item[targetProperty].indexOf(account.id) === -1)
              throw new UnauthorizedException(
                `Not allowed operate this item on account ${args[payloadIndex][targetProperty]}, please contact the administrator`,
              );
          } else {
            if (item[targetProperty] !== account.id)
              throw new UnauthorizedException(
                `Not allowed operate this item on account ${args[payloadIndex][targetProperty]}, please contact the administrator`,
              );
          }
