  ...
  implements IamIAccountRepository
{
  ...
  async createdAggregateHook(
    ...
    if (aggregate?.roleIds.length > 0) {
      try {
        await model.$add('roles', aggregate.roleIds.value, createOptions);
      } catch (error) {
    ...
    if (aggregate?.tenantIds.length > 0) {
      try {
        await model.$add('tenants', aggregate.tenantIds.value, createOptions);
      } catch (error) {
        console.error('[Error] SequelizeRepository:', error);
      }
    }
  ...
  async updatedByIdAggregateHook(
    ...
    if (aggregate?.roleIds.isArray()) {
      try {
        await model.$set('roles', aggregate.roleIds.value, updateByIdOptions);
      } catch (error) {
        console.error('[Error] SequelizeRepository:', error);
      }
    }
