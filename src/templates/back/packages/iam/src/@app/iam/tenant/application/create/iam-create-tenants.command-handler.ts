...
export class IamCreateTenantsCommandHandler
  implements ICommandHandler<IamCreateTenantsCommand>
{
  constructor(private readonly createTenantsService: IamCreateTenantsService) {}

  async execute(command: IamCreateTenantsCommand): Promise<void> {
    // call to use case and implements ValueObjects
    await this.createTenantsService.main(
      command.payload.map((tenant) => {
        return {
