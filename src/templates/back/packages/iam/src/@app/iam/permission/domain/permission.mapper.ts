...
export class PermissionMapper implements IMapper
{
    ...
    private makeAggregate(permission: LiteralObject, cQMetadata?: CQMetadata): IamPermission
    {
        return IamPermission.register(
            ...
            this.options.eagerLoading ? new BoundedContextMapper({ eagerLoading: true }).mapModelToAggregate(permission.boundedContext, cQMetadata) : undefined,
            this.options.eagerLoading ? new RoleMapper({ eagerLoading: true }).mapModelsToAggregates(permission.roles, cQMetadata) : undefined,
        );
    }
