...
export class IamCreateUsersCommandHandler
    implements ICommandHandler<IamCreateUsersCommand>
{
    constructor(private readonly createUsersService: IamCreateUsersService) {}

    async execute(command: IamCreateUsersCommand): Promise<void> {
        // call to use case and implements ValueObjects
        await this.createUsersService.main(
            command.payload.map((user) => {
                return {
