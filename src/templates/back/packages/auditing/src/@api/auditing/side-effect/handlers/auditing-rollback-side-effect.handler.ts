    ...
    async main(
        ...
        switch (sideEffect.event) {
            case AuditingSideEffectEvent.CREATED:
                const objectCreated = await m[sideEffect.modelName].findByPk(
                ...
                if (!objectCreated)
                    throw new BadRequestException(
                        `The object from model ${sideEffect.modelName} with id ${sideEffect.auditableId} no longer exists in the database`,
                    );
            ...
            case AuditingSideEffectEvent.UPDATED:
                const objectUpdated = await m[sideEffect.modelName].findByPk(
                    sideEffect.auditableId,
                );
            ...
            case AuditingSideEffectEvent.DELETED:
                await m[sideEffect.modelName].create(
                    {
                        ...sideEffect.oldValue,
                        createdAt: now,
                        updatedAt: now,
