import {
  ...
  AuditingUpdateHttpCommunicationByIdDto,
...
import {
  ...
  AuditingUpdateHttpCommunicationByIdCommand,
...
export class AuditingUpdateHttpCommunicationByIdHandler {
  ...
  async main(
    ...
    const httpCommunication = await this.queryBus.ask(
      new AuditingFindHttpCommunicationByIdQuery(payload.id, constraint, {
        timezone,
      }),
    );
