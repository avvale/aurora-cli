...
export class McpController {
    ...
    constructor(private readonly mcp: McpNestGraphQLServer) {}
    ...
    async handle(@Req() req: Request, @Res() res: Response): Promise<void> {
        ...
        const method =
            req.body && typeof req.body === 'object' && 'method' in req.body
                ...
                : 'unknown';
        this.logger.debug(
            `MCP HTTP request - method=${method} sessionId=${sessionId ?? 'none'} transports=${this.transports.size}`,
        );
        ...
        if (!transport) {
            if (!isInitializeRequest(req.body)) {
                const id =
                    req.body && typeof req.body === 'object' && 'id' in req.body
                        ...
                        : null;
                this.logger.warn(
                    `Request without valid MCP session. Method: ${(req.body && req.body.method) || 'unknown'} (GraphQL MCP).`,
                );
                res.status(200).json({
                    jsonrpc: '2.0',
                    id,
                    error: {
                        code: -32000,
                        message:
                            'Session not initialized or expired. Please call initialize.',
                    },
                });
                return;
            }
            ...
            transport = new StreamableHTTPServerTransport({
                ...
                onsessioninitialized: (sid: string) => {
                    this.logger.log(
                        `MCP (GraphQL) session initialized: ${sid}`,
                    );
                ...
                onsessionclosed: (sid: string) => {
                    this.logger.log(
                        `MCP (GraphQL) session closed (callback): ${sid}`,
                    );
            ...
            transport.onclose = () => {
                if (transport?.sessionId) {
                    this.logger.log(
                        `MCP (GraphQL) session closed: ${transport.sessionId}`,
                    );
        ...
        try {
            ...
            this.logger.error(
                `Error handling MCP (GraphQL) request: ${message}`,
            );
            if (!res.headersSent) {
                res.status(200).json({
                    jsonrpc: '2.0',
