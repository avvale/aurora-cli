...
import { {{ toPascalCase (getRelationshipBoundedContextName this) }}{{ toPascalCase getRelationshipModuleName }}Mapper } from '{{#if relationship.packageName }}{{ relationship.packageName }}{{else}}{{ config.appContainer }}/{{ relationship.modulePath }}{{/if}}';
{{/each}}
...
export class {{ toPascalCase schema.boundedContextName }}{{ toPascalCase schema.moduleName }}Mapper implements IMapper
{
    ...
    private makeAggregate({{ toCamelCase schema.moduleName }}: LiteralObject, cQMetadata?: CQMetadata): {{ schema.aggregateName }}
    {
        return {{ schema.aggregateName }}.register(
            ...
            {{#if hasTimezone}}
            new {{ toPascalCase schema.boundedContextName }}{{ toPascalCase ../schema.moduleName }}{{> i18n }}{{ toPascalCase name }}({{ toCamelCase ../schema.moduleName }}{{#if isI18n}}.{{ toCamelCase ../schema.moduleName }}{{> i18n }}{{/if}}.{{ toCamelCase (getNameProperty this) }}, { undefinable: true }, { addTimezone: cQMetadata?.timezone }),
            {{else}}
            new {{ toPascalCase schema.boundedContextName }}{{ toPascalCase ../schema.moduleName }}{{> i18n }}{{ toPascalCase name }}({{ toCamelCase ../schema.moduleName }}{{#if isI18n}}.{{ toCamelCase ../schema.moduleName }}{{> i18n }}{{/if}}.{{ toCamelCase (getNameProperty this) }}, { undefinable: true }),
            {{/if}}
            {{/if}}
            {{/each}}
            {{#each schema.aggregateProperties.withRelationshipOneToOneWithRelationshipField}}
            this.options.eagerLoading ? new {{ toPascalCase (getRelationshipBoundedContextName this) }}{{ toPascalCase getRelationshipModuleName }}Mapper({ eagerLoading: true }).mapModelToAggregate({{ toCamelCase ../schema.moduleName }}.{{#if isI18n}}{{ toCamelCase ../schema.moduleName }}I18n.{{/if}}{{ toCamelCase relationship.field }}, cQMetadata) : undefined,
            {{/each}}
            {{#each schema.aggregateProperties.withRelationshipOneToOneWithoutRelationshipField}}
            this.options.eagerLoading ? new {{ toPascalCase (getRelationshipBoundedContextName this) }}{{ toPascalCase getRelationshipModuleName }}Mapper({ eagerLoading: true }).mapModelToAggregate({{ toCamelCase ../schema.moduleName }}.{{#if isI18n}}{{ toCamelCase ../schema.moduleName }}I18n.{{/if}}{{ toCamelCase originName }}, cQMetadata) : undefined,
            {{/each}}
            ...
            this.options.eagerLoading ? new {{ toPascalCase (getRelationshipBoundedContextName this) }}{{ toPascalCase getRelationshipModuleName }}Mapper({ eagerLoading: true }).mapModelsToAggregates({{ toCamelCase ../schema.moduleName }}.{{#if isI18n}}{{ toCamelCase ../schema.moduleName }}I18n.{{/if}}{{ toCamelCase originName }}, cQMetadata) : undefined,
            {{/each}}
    ...
    private makeResponse({{ toCamelCase schema.moduleName }}: {{ schema.aggregateName }}): {{ toPascalCase schema.boundedContextName }}{{ toPascalCase schema.moduleName }}Response
    {
        ...
        return new {{ toPascalCase schema.boundedContextName }}{{ toPascalCase schema.moduleName }}Response(
            ...
            {{/each}}
            {{#each schema.aggregateProperties.withRelationshipOneToOneWithRelationshipField}}
            this.options.eagerLoading ? new {{ toPascalCase (getRelationshipBoundedContextName this) }}{{ toPascalCase getRelationshipModuleName }}Mapper({ eagerLoading: true }).mapAggregateToResponse({{ toCamelCase ../schema.moduleName }}.{{ toCamelCase relationship.field }}) : undefined,
            {{/each}}
            {{#each schema.aggregateProperties.withRelationshipOneToOneWithoutRelationshipField}}
            this.options.eagerLoading ? new {{ toPascalCase (getRelationshipBoundedContextName this) }}{{ toPascalCase getRelationshipModuleName }}Mapper({ eagerLoading: true }).mapAggregateToResponse({{ toCamelCase ../schema.moduleName }}.{{ toCamelCase originName }}) : undefined,
            {{/each}}
            {{#each schema.aggregateProperties.withRelationshipManyToOne}}
            {{#unless (isI18nRelationProperty ../schema.moduleName this)}}
            this.options.eagerLoading ? new {{ toPascalCase (getRelationshipBoundedContextName this) }}{{ toPascalCase getRelationshipModuleName }}Mapper({ eagerLoading: true }).mapAggregateToResponse({{ toCamelCase ../schema.moduleName }}.{{ toCamelCase relationship.field }}) : undefined,
            ...
            {{/each}}
            {{#each schema.aggregateProperties.withRelationshipOneToMany}}
            this.options.eagerLoading ? new {{ toPascalCase (getRelationshipBoundedContextName this) }}{{ toPascalCase getRelationshipModuleName }}Mapper({ eagerLoading: true }).mapAggregatesToResponses({{ toCamelCase ../schema.moduleName }}.{{ toCamelCase originName }}) : undefined,
            {{/each}}
            {{#each schema.aggregateProperties.withRelationshipManyToMany}}
            this.options.eagerLoading ? new {{ toPascalCase (getRelationshipBoundedContextName this) }}{{ toPascalCase getRelationshipModuleName }}Mapper({ eagerLoading: true }).mapAggregatesToResponses({{ toCamelCase ../schema.moduleName }}.{{ toCamelCase originName }}) : undefined,
            {{/each}}
