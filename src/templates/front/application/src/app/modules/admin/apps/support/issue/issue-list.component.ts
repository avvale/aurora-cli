        ...
        switch (action?.id)
        {
                ...
                await lastValueFrom(
                    this.issueService.pagination({
                        query: action.meta.query ?
                            action.meta.query :
                            queryStatementHandler({ columnsConfig: issueColumnsConfig })
            ...
            case 'support::issue.list.delete':
                const deleteDialogRef = this.confirmationService.open({
                    ...
                    message: this.translocoService.translate('DeletionWarning', { entity: this.translocoService.translate('support.Issue') }),
                    icon   : {
                        show : true,
            ...
            case 'support::issue.list.export':
                ...
                const columns: string[] = issueColumnsConfig.map(issueColumnConfig => issueColumnConfig.field);
                const headers: string[] = issueColumnsConfig.map(issueColumnConfig => this.translocoService.translate(issueColumnConfig.translation));
