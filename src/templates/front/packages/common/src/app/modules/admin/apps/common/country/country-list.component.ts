...
@ActionScope('common::country.list')
export class CountryListComponent extends ViewBaseComponent {
  ...
  originColumnsConfig: ColumnConfig[] = [
    ...
    ...countryColumnsConfig({ translator: this.translocoService }),
  ];
    ...
    switch (action?.id) {
      ...
      case 'common::country.list.new':
        this.router.navigate([
          ...
          action.meta.row.id,
          action.meta.lang &&
            action.meta.lang[this.sessionService.get<string>('searchKeyLang')],
        ]);
        break;
      ...
      case 'common::country.list.pagination':
        await lastValueFrom(
          this.countryService.pagination({
            ...
            headers: {
              'Content-Language':
                this.sessionService.get('fallbackLang')[
                  this.sessionService.get('searchKeyLang')
                ],
            },
          }),
        );
        ...
        this.router.navigate([
          ...
          action.meta.lang
            ...
            : this.sessionService.get<CoreLang>('fallbackLang')[
                this.sessionService.get<string>('searchKeyLang')
              ],
        ]);
        break;
      ...
      case 'common::country.list.export':
        ...
        const columns: string[] = countryColumnsConfig().map(
          (countryColumnConfig) => countryColumnConfig.field,
        );
        const headers: string[] = countryColumnsConfig().map(
          (countryColumnConfig) =>
            this.translocoService.translate(countryColumnConfig.translation),
        );
