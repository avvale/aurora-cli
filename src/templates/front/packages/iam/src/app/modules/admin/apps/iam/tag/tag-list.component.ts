    ...
    async handleAction(action: Action): Promise<void> {
        ...
        switch (action?.id) {
                ...
                await lastValueFrom(
                    this.tagService.pagination({
                        query: action.meta.query
                            ? action.meta.query
                            : queryStatementHandler({
                                  columnsConfig: tagColumnsConfig,
                              })
                                  .setColumFilters(
                                      this.gridFiltersStorageService.getColumnFilterState(
                                          this.gridId,
            ...
            case 'iam::tag.list.edit':
                this.router.navigate(['iam/tag/edit', action.meta.row.id]);
                break;
            ...
            case 'iam::tag.list.delete':
                const deleteDialogRef = this.confirmationService.open({
                    ...
                    message: this.translocoService.translate(
                        'DeletionWarning',
                        { entity: this.translocoService.translate('iam.Tag') },
                    ),
                    icon: {
                        show: true,
                ...
                deleteDialogRef.afterClosed().subscribe(async (result) => {
                    if (result === 'confirmed') {
                        try {
                            await lastValueFrom(
                                this.tagService.deleteById<IamTag>({
            ...
            case 'iam::tag.list.export':
                ...
                const columns: string[] = tagColumnsConfig.map(
                    (tagColumnConfig) => tagColumnConfig.field,
                );
