...
import {
    checkScriptProcedureMutation,
    downScriptProcedureMutation,
    runScriptsProcedureMutation,
    upScriptProcedureMutation,
} from './procedure.graphql';

@Injectable({
    providedIn: 'root',
})
export class ProcedureService {
    paginationSubject$: BehaviorSubject<GridData<ToolsProcedure> | null> =
        new BehaviorSubject(null);
    procedureSubject$: BehaviorSubject<ToolsProcedure | null> =
        new BehaviorSubject(null);
    proceduresSubject$: BehaviorSubject<ToolsProcedure[] | null> =
        new BehaviorSubject(null);

    // scoped subjects
    paginationScoped: {
        [key: string]: BehaviorSubject<GridData<ToolsProcedure> | null>;
    } = {};
    procedureScoped: { [key: string]: BehaviorSubject<ToolsProcedure | null> } =
        {};
    proceduresScoped: {
        [key: string]: BehaviorSubject<ToolsProcedure[] | null>;
    } = {};
    ...
    setScopePagination(
        scope: string,
        pagination: GridData<ToolsProcedure>,
    ): void {
    ...
    findById({
        ...
        return this.graphqlService
            ...
            .valueChanges.pipe(
                ...
                tap((data) =>
                    scope
                        ? this.setScopeProcedure(scope, data.object)
                        : this.procedureSubject$.next(data.object),
                ),
            );
    }
    ...
    get({
        ...
        return this.graphqlService
            ...
            .valueChanges.pipe(
                ...
                tap((data) =>
                    scope
                        ? this.setScopeProcedures(scope, data.objects)
                        : this.proceduresSubject$.next(data.objects),
                ),
            );
    }
