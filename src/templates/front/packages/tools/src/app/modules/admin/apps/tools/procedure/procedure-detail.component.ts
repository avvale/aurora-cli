    ...
    onSubmit($event): void {
        ...
        this.actionService.action({
            id: mapActions(this.currentViewAction.id, {
                ...
                'tools::procedure.detail.edit':
                    'tools::procedure.detail.update',
            }),
            isViewAction: false,
        });
    }
    ...
    async handleAction(action: Action): Promise<void> {
        ...
        switch (action?.id) {
            ...
            case 'tools::procedure.detail.create':
                try {
                    await lastValueFrom(
                        this.procedureService.create<ToolsProcedure>({
            ...
            case 'tools::procedure.detail.update':
                try {
                    await lastValueFrom(
                        this.procedureService.updateById<ToolsProcedure>({
                            object: this.fg.value,
                        }),
                    );
            ...
            case 'tools::procedure.detail.upScript':
                ...
                deleteDialogRef.afterClosed().subscribe(async (result) => {
                    if (result === 'confirmed') {
                        try {
                            await lastValueFrom(
                                this.procedureService.upScriptProcedure<ToolsProcedure>(
                                    {
            ...
            case 'tools::procedure.detail.downScript':
                ...
                downScriptDialogRef.afterClosed().subscribe(async (result) => {
                    if (result === 'confirmed') {
                        try {
                            await lastValueFrom(
                                this.procedureService.downScriptProcedure<ToolsProcedure>(
                                    {
                                        procedureId: this.managedObject().id,
                                    },
                                ),
