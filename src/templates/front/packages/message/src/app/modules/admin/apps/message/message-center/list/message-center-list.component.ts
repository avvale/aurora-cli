...
export const messageCenterPaginationListAction =
    'message::messageCenter.list.pagination';
export const messageCenterExportListAction =
    'message::messageCenter.list.export';
export const messageCustomerCenterMessageScope =
    'message::customerCenterMessage';
...
export class MessageCenterListComponent extends ViewBaseComponent {
    ...
    lastMessageOfPage = computed(() =>
        this.currentPage() * this.limit + this.limit > this.totalMessages()
            ? this.totalMessages()
    ...
    init(): void {
        ...
        this.messageService.toggleMessageAsRead$
            .pipe(takeUntil(this.unsubscribeAll$))
            .subscribe((toggleMessage: MessageInbox) => {
                const messages = this.messages();
                this.messages.set(
                    messages.map((message) => {
                        if (message.id === toggleMessage.id)
        ...
        this.messageService.deletedMessage$
            .pipe(takeUntil(this.unsubscribeAll$))
