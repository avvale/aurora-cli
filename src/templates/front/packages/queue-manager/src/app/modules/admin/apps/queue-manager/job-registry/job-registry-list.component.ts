...
import { QueueManagerJobRegistry } from '@apps/queue-manager/queue-manager.types';
import {
    Action,
    ColumnConfig,
    ColumnDataType,
...
export const jobRegistryMainGridListId =
    'queueManager::jobRegistry.list.mainGridList';
...
export class JobRegistryListComponent extends ViewBaseComponent {
    ...
    gridId: string = jobRegistryMainGridListId;
        ...
        switch (action?.id) {
            ...
            case 'queueManager::jobRegistry.list.edit':
                this.router.navigate([
                    'queue-manager/job-registry/edit',
                    action.meta.row.id,
                ]);
                break;
            ...
            case 'queueManager::jobRegistry.list.delete':
                ...
                deleteDialogRef.afterClosed().subscribe(async (result) => {
                    if (result === 'confirmed') {
                        try {
                            await lastValueFrom(
                                this.jobRegistryService.deleteById<QueueManagerJobRegistry>(
                                    {
            ...
            case 'queueManager::jobRegistry.list.export':
                const rows = await lastValueFrom(
                    this.jobRegistryService.get({
                        query: action.meta.query,
                    }),
                );

                const columns: string[] = jobRegistryColumnsConfig.map(
                    (jobRegistryColumnConfig) => jobRegistryColumnConfig.field,
                );
                const headers: string[] = jobRegistryColumnsConfig.map(
                    (jobRegistryColumnConfig) =>
                        this.translocoService.translate(
                            jobRegistryColumnConfig.translation,
                        ),
                );
