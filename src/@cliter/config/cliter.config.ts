...
export interface CliterConfig {
    ...
    propertyTypesEquivalenceDtoTypes: {
        [key: string]: (property?: Property, config?: CliterConfig) => string;
    };
...
export const cliterConfig: CliterConfig = {
    ...
    propertyTypesEquivalenceDtoTypes: {
        ...
        'blob.medium': () => 'string',
        'blob.tiny': () => 'string',
        array: (property?: Property, config?: CliterConfig) =>
            config!.propertyTypesEquivalenceDtoTypes[
                property!.arrayOptions!.type
            ]() + '[]',
        bigint: () => 'number',
        blob: () => 'string',
        boolean: () => 'boolean',
    ...
    propertyTypesEquivalenceSequelizeTypes: {
        ...
        char: (length: number): string =>
            `DataTypes.CHAR(${length?.toString()})`,
        date: (length: number): string => 'DataTypes.DATE',
        decimal: (decimals: number[]): string =>
            `DataTypes.DECIMAL(${decimals.join(',')})`,
        encrypted: (length: number): string => 'DataTypes.TEXT',
        enum: (property: Property): string =>
            `DataTypes.ENUM(${getPropertyStringEnumOptions(property)})`,
        ...
        password: (length: number): string =>
            `DataTypes.STRING(${length?.toString()})`,
        ...
        varchar: (property: Property): string =>
            `DataTypes.STRING(${property?.maxLength ? property.maxLength.toString() : ''})`,
        array: (parameter: any): string =>
            `DataTypes.ARRAY(${cliterConfig.propertyTypesEquivalenceSequelizeTypes[parameter.type](parameter)})`,
    },
    defaultTypeLength: {
        bigint: 20,
        char: 127,
