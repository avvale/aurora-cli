...
import {
  getEnumProperties,
  getForeignRelationshipProperties,
  getGraphqlInputProperties,
  getGraphqlProperties,
  getWithoutTimestampsProperties,
  hasI18nProperties,
} from '../properties.functions';
import {
  getPropertyEnumOptions,
  getPropertyJavascriptCreateType,
  getPropertyJavascriptType,
  getPropertyJavascriptUpdateType,
} from '../property.functions';
import { ArrayDriver } from './drivers/array.driver';
import { EnumDriver } from './drivers/enum.driver';
  ...
  generateBackAppBoundedContextReferences(
    ...
    const sourceFile = this.project.addSourceFileAtPath(
      path.join(
        ...
        this.srcDirectory,
        this.appDirectory,
        this.boundedContextName.toKebabCase(),
        'public-api.ts',
      ),
    );
        ...
        `
{
  ...
  declareBackApplicationItemsInModule(): void {
    ...
    const sourceFile = this.project.addSourceFileAtPath(
      path.join(
        ...
        this.apiDirectory,
        this.boundedContextName.toKebabCase(),
        `${this.boundedContextName.toKebabCase()}.module.ts`,
      ),
    );
    const moduleDecoratorArguments = this.getModuleDecoratorArguments(
      sourceFile,
    ...
    // providers
    const providers: InitializerExpressionGetableNode =
      moduleDecoratorArguments.getProperty(
        'providers',
      ) as InitializerExpressionGetableNode;
    const providersArray: ArrayLiteralExpression = <ArrayLiteralExpression>(
      getInitializer(providers)
    );

    // controllers
    const controllers: InitializerExpressionGetableNode =
      moduleDecoratorArguments.getProperty(
        'controllers',
      ) as InitializerExpressionGetableNode;
    const controllersArray = <ArrayLiteralExpression>(
      getInitializer(controllers)
    );
    ...
    const modelArrayArgument = this.getModelArrayArgument(
      moduleDecoratorArguments,
    );
    ArrayDriver.addArrayItem(
      sourceFile,
      `...${this.boundedContextName.toPascalCase()}Models`,
  ...
  generateBackTestingForeignReferences(properties: Property[]): void {
    ...
    const foreignRelationships = getForeignRelationshipProperties(
      properties,
    ...
    const yamlFiles = fs.readdirSync(
      path.join(process.cwd(), 'cliter', this.boundedContextName.toKebabCase()),
    );

    for (const foreignRelationship of foreignRelationships) {
      if (!foreignRelationship.relationship?.modulePath) return;

      const foreignBoundedContextName =
        foreignRelationship.relationship?.modulePath.split('/')[0];

      for (const yamlFile of yamlFiles) {
        // get file name of e2e test
        const e2eTestFile = yamlFile.replace(
          cliterConfig.schemaDefinitionExtension,
          '.e2e-spec.ts',
        );
  ...
  declareBackBoundedContextModuleInApplicationModule(): void {
    const sourceFile = this.project.addSourceFileAtPath(
      path.join(process.cwd(), this.srcDirectory, 'app.module.ts'),
    );
    const moduleDecoratorArguments = this.getModuleDecoratorArguments(
      sourceFile,
      'AppModule',
    ...
    if (!modules.includes(`${this.boundedContextName.toPascalCase()}Module`)) {
      ...
      // register module
      const importsArgument: InitializerExpressionGetableNode =
        moduleDecoratorArguments.getProperty(
          'imports',
        ) as InitializerExpressionGetableNode;
      const importsArray = <ArrayLiteralExpression>(
        getInitializer(importsArgument)
      );
      ArrayDriver.addArrayItems(
        sourceFile,
        [`${this.boundedContextName.toPascalCase()}Module`],
  ...
  generateFrontInterface(
    ...
    const sourceFile = this.project.addSourceFileAtPath(
      path.join(
        process.cwd(),
        this.srcDirectory,
        cliterConfig.dashboardContainer,
        this.boundedContextName.toKebabCase(),
        `${this.boundedContextName.toKebabCase()}.types.ts`,
      ),
    );

    // generate enums for properties of type ENUM
    for (const property of getEnumProperties(properties)) {
      const enumOptions = getPropertyEnumOptions(property);
    ...
    for (const property of getGraphqlProperties(properties)
      ...
      if (property.relationship?.type === RelationshipType.MANY_TO_ONE) {
        propertiesObjectType.push({
          name:
            property.relationship.field.toCamelCase() +
            (property.nullable ? '?' : ''),
  ...
  generateFrontRoutes(index = 0): void {
    const sourceFile = this.project.addSourceFileAtPath(
      path.join(
        ...
        cliterConfig.dashboardContainer,
        this.boundedContextName.toKebabCase(),
        `${this.boundedContextName.toKebabCase()}.routes.ts`,
      ),
    );
    const defaultExportDeclarations = sourceFile.getExportedDeclarations();
    const routesArray = (
      defaultExportDeclarations.get('default') as ArrayLiteralExpression[]
    )[0];
    const objectRoute = routesArray.getElements()[
      index
    ] as ObjectLiteralExpression;
    const childrenRoutes = objectRoute.getPropertyOrThrow(
      'children',
    ) as InitializerExpressionGetableNode;
    const childrenRoutesArray = <ArrayLiteralExpression>(
      getInitializer(childrenRoutes)
    );
    ...
    ArrayDriver.addArrayItem(
      ...
      (item: string, array: ArrayLiteralExpression | undefined) => {
        const foundItem = (
          array?.getElements() as ObjectLiteralExpression[]
        ).find((item) => {
          return (
            `'${this.moduleName.toKebabCase()}'` ===
            (<StringLiteral>(
              getInitializer(
                item.getPropertyOrThrow(
                  'path',
                ) as InitializerExpressionGetableNode,
              )
            )).getText()
    ...
    ArrayDriver.addArrayItem(
      ...
      (item: string, array: ArrayLiteralExpression | undefined) => {
        const foundItem = (
          ...
          return (
            `'${this.moduleName.toKebabCase()}/new'` ===
            (<StringLiteral>(
              getInitializer(
                item.getPropertyOrThrow(
                  'path',
      ...
      ArrayDriver.addArrayItem(
        ...
        (item: string, array: ArrayLiteralExpression | undefined) => {
          const foundItem = (
            ...
            return (
              `'${this.moduleName.toKebabCase()}/new/:id/:langId'` ===
              (<StringLiteral>(
                getInitializer(
                  item.getPropertyOrThrow(
                    'path',
      ...
      ArrayDriver.addArrayItem(
        ...
        (item: string, array: ArrayLiteralExpression | undefined) => {
          const foundItem = (
            ...
            return (
              `'${this.moduleName.toKebabCase()}/edit/:id/:langId'` ===
              (<StringLiteral>(
                getInitializer(
                  item.getPropertyOrThrow(
                    'path',
    ...
    ArrayDriver.addArrayItem(
      ...
      (item: string, array: ArrayLiteralExpression | undefined) => {
        const foundItem = (
          ...
          return (
            `'${this.moduleName.toKebabCase()}/edit/:id'` ===
            (<StringLiteral>(
              getInitializer(
                item.getPropertyOrThrow(
                  'path',
  ...
  declareFrontComponents(): void {
    ...
    // declarations
    const declarations: InitializerExpressionGetableNode =
      moduleDecoratorArguments.getProperty(
        'declarations',
      ) as InitializerExpressionGetableNode;
    const declarationsArray: ArrayLiteralExpression = <ArrayLiteralExpression>(
      getInitializer(declarations)
    );
  ...
  declareFrontBoundedContext(index = 5): void {
    const sourceFile = this.project.addSourceFileAtPath(
      path.join(process.cwd(), this.srcDirectory, 'app', 'app.routes.ts'),
    );
    ...
    const objectRoute = appRoutesArray.getElements()[
      index
    ] as ObjectLiteralExpression;
    const childrenRoutes = objectRoute.getPropertyOrThrow(
      'children',
    ...
    const childrenRoutesElements =
      childrenRoutesArray.getElements() as ObjectLiteralExpression[];

    // avoid duplicated declaration
    for (const element of childrenRoutesElements) {
      const pathProperty = element.getPropertyOrThrow(
        'path',
      ) as InitializerExpressionGetableNode;
      const pathString = <StringLiteral>getInitializer(pathProperty);

      if (pathString.getText() === `'${this.boundedContextName.toKebabCase()}'`)
        return;
    }
  ...
  generateFrontNavigation(
    ...
    const sourceFile = this.project.addSourceFileAtPath(
      path.join(
        ...
        `${boundedContextName.toKebabCase()}.navigation.ts`,
      ),
    );
    const navigation = sourceFile.getVariableDeclarationOrThrow(
      boundedContextName.toCamelCase() + 'Navigation',
    );
    const navigationObject = <ObjectLiteralExpression>(
      getInitializer(navigation)
    );
    const childrenProperty = navigationObject.getPropertyOrThrow(
      'children',
    ) as InitializerExpressionGetableNode;
    const childrenArrayNavigation = <ArrayLiteralExpression>(
      getInitializer(childrenProperty)
    );
    const navigationElements =
      childrenArrayNavigation.getElements() as ObjectLiteralExpression[];
  ...
  generateFrontTranslations(properties: Property[], langCode: string): void {
    const translationObject = require(
      path.join(
        process.cwd(),
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(newTranslationObject, null, 4),
    );
  }
  ...
  generateFrontNavigationTranslation(langCode: string): void {
    ...
    if (translationObject[this.moduleNames.toCamelCase()] === undefined) {
      translationObject[this.moduleNames.toCamelCase()] =
        this.moduleNames.toPascalCase();
    }
  ...
  private getModelArrayArgument(
    ...
    const importsArray: ArrayLiteralExpression = <ArrayLiteralExpression>(
      getInitializer(importsArgument)
    );
    const importsElements = importsArray.getElements();
    const SequelizeModuleElement: CallExpression = importsElements.find(
      (el) => el.getText().indexOf('SequelizeModule.forFeature') === 0,
    ) as CallExpression;

    if (!SequelizeModuleElement)
      throw new Error(
        'SequelizeModule.forFeature is not defined in the module, it has to be defined to get it.',
      );
  ...
  private getModuleDecoratorArguments(
    ...
    const moduleDecorator: Decorator = moduleClass?.getDecorator(
      decorator,
    ) as Decorator;
