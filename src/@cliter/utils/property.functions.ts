...
export const getPropertyName = (property: Property): string => {
  ...
  if (property.relationship?.type === RelationshipType.MANY_TO_MANY)
    return `${property.relationship.singularName}Ids`;
  return property.name;
};

// replace by Property hasTimezone
export const isTimezoneProperty = (property: Property): boolean => {
...
export const getPropertySequelizeType = (
  ...
  if (property.type === PropertyType.CHAR && property?.length)
    parameter = property.length; // parameter = length
  ...
  return config.propertyTypesEquivalenceSequelizeTypes[property.type](
    parameter,
  );
};
...
export const getJavascriptModelTypeProperty = (
  ...
  if (property.relationship?.type === RelationshipType.MANY_TO_MANY)
    return config.propertyTypesEquivalenceJavascriptTypes.manyToMany;
  if (property.type === PropertyType.RELATIONSHIP)
    return `${property.relationship?.aggregateName}[]`;
  if (property.type === PropertyType.ARRAY && property.arrayOptions)
    return `${config.propertyTypesEquivalenceJavascriptModelTypes[property.arrayOptions.type]}[]`;

  return config.propertyTypesEquivalenceJavascriptModelTypes[property.type];
};

// replace by Property getDefaultValue
export const getDefaultValueProperty = (property: Property): any => {
...
export const getReferenceKeyProperty = (property: Property): any => {
...
export const getPropertyStringEnumOptions = (
  ...
  return getPropertyEnumOptions(property)
...
export const getPropertyEnumOptions = (
  ...
  if (
    ...
    return property.arrayOptions.enumOptions?.map((option: string) =>
      option.trim().toUpperCase(),
    );
  return property.enumOptions?.map((option: string) =>
    option.trim().toUpperCase(),
  );
};

// replace by Property isRelationship
export const isRelationshipProperty = (property: Property): boolean => {
...
export const isPrimaryKeyProperty = (property: Property): boolean => {
  return Boolean(property.primaryKey);
};

// replace by Property isBinary
export const isBinaryProperty = (property: Property): boolean => {
  return (
    property.type === PropertyType.BLOB ||
    property.type === PropertyType.MEDIUMBLOB ||
    property.type === PropertyType.LONGBLOB
  );
};
...
export const hasColumnDecoratorProperty = (property: Property): boolean => {
  return (
...
export const hasHasOneDecoratorProperty = (property: Property): boolean => {
  return (
    property.relationship?.type === RelationshipType.ONE_TO_ONE &&
    !property.relationship.field
  );
...
export const hasBelongsToDecoratorProperty = (property: Property): boolean => {
  return (
...
export const hasHasManyDecoratorProperty = (property: Property): boolean => {
  return property.relationship?.type === RelationshipType.ONE_TO_MANY;
};
...
export const isPivotProperty = (property: Property): boolean => {
...
export const getAggregateNameFromPropertyRelationship = (
  ...
  try {
    ...
    throwRelationshipEntityNotCreatedProperty(modulePath, 'aggregateName');
  }
...
export const getBoundedContextNameFromPropertyRelationship = (
  ...
  try {
    ...
    throwRelationshipEntityNotCreatedProperty(modulePath, 'boundedContextName');
  }
...
export const getModuleNameFromPropertyRelationship = (
  ...
  try {
    ...
    throwRelationshipEntityNotCreatedProperty(modulePath, 'moduleName');
  }
...
export const getModuleNamesFromPropertyRelationship = (
  ...
  try {
    ...
    throwRelationshipEntityNotCreatedProperty(modulePath, 'moduleNames');
  }
...
export const getSchemaFromPropertyRelationship = (
  ...
  try {
    ...
    throwRelationshipEntityNotCreatedProperty(modulePath, 'schema');
  }
...
export const getGraphqlTypeProperty = (
  ...
  if (property.relationship?.type === RelationshipType.MANY_TO_ONE)
    return `${property.relationship?.aggregateName}`;
  if (property.relationship?.type === RelationshipType.ONE_TO_ONE)
    return `${property.relationship?.aggregateName}`;
  if (property.type === PropertyType.ARRAY && property.arrayOptions)
    return `[${config.propertyTypesEquivalenceQraphqlTypes[property.arrayOptions.type]}]`;
...
export const getGraphqlCreateTypeProperty = (
  ...
  if (property.relationship?.type === RelationshipType.MANY_TO_MANY)
    return config.propertyTypesEquivalenceQraphqlTypes.manyToMany;
  if (
    property.relationship?.type === RelationshipType.ONE_TO_ONE &&
    !property.relationship.field
  )
    return `${getBoundedContextNameFromPropertyRelationship(property.relationship.modulePath)?.toPascalCase()}Create${getModuleNameFromPropertyRelationship(property.relationship.modulePath)?.toPascalCase()}Input`;
  if (property.type === PropertyType.ARRAY && property.arrayOptions)
    return `[${config.propertyTypesEquivalenceQraphqlTypes[property.arrayOptions.type]}]`;
...
export const getGraphqlUpdateTypeProperty = (
  ...
  if (
    ...
    return `${getBoundedContextNameFromPropertyRelationship(property.relationship.modulePath)?.toPascalCase()}Update${getModuleNameFromPropertyRelationship(property.relationship.modulePath)?.toPascalCase()}Input`;
  if (property.type === PropertyType.ARRAY && property.arrayOptions)
    return `[${config.propertyTypesEquivalenceQraphqlTypes[property.arrayOptions.type]}]`;
...
export const getEnumUnionLiteralType = (property: Property): string => {
...
export const getPropertyJavascriptCreateType = (
  ...
  if (property.type === PropertyType.RELATIONSHIP)
    return `${property.relationship?.aggregateName}[]`;
  if (property.type === PropertyType.ARRAY && property.arrayOptions)
    return `${config.propertyTypesEquivalenceJavascriptTypes[property.arrayOptions.type]}[]`;
  if (property.type === PropertyType.ENUM)
    return getEnumUnionLiteralType(property);
