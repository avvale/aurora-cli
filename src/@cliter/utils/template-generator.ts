...
export class TemplateGenerator {
  ...
  static async generateValueObject(
    ...
    if (
      !excludeFiles(excludedFiles).isAllowPath(
        path.join(relativeTargetBasePath, relativeTargetPath, nameReplaced),
      )
    ) {
      command.log(
    ...
    if (!fs.existsSync(originFilePath))
      throw new Error(
        'Value object not exist, must to create template ' + originFilePath,
      );
  ...
  static async generateType(
    ...
    // handle array type with enum inside arrayOptions
    const isArrayWithEnum =
      property.type === PropertyType.ARRAY &&
      property.arrayOptions?.type === PropertyType.ENUM;
    const templateType = isArrayWithEnum ? PropertyType.ENUM : property.type;

    // if array contains enum, copy enumOptions from arrayOptions to property root for template compatibility
    const propertyForTemplate = isArrayWithEnum
      ? { ...property, enumOptions: property.arrayOptions?.enumOptions }
      : property;
    ...
    if (!fs.existsSync(originFilePath))
      throw new Error(
        'Type template does not exist, must create template ' + originFilePath,
      );
